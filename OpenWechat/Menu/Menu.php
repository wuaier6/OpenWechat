 ...
 */
namespace App\Common\OpenWechat\Menu;
...
class Menu extends AbstractAPI
{
     ...
     * @return bool
     */
    public function add($appid,array $buttons, array $matchRule = [])
    {
        $access_token = $this->auth->getAuthorizerToken($appid);
        if (!empty($matchRule)) {
        ...
        return $this->parseJSON('json', [self::API_CREATE.$access_token, ['button' => $buttons]]);
    }
